...
class dataLoader:
    def __init__(self,basepath,part,no_frames,batchsize,fsize=128,isTrain=True,isVid=False):
        ...
        if isTrain:
            if isVid:
                ...
                self.filelistX,self.filelistY,self.filelistM = self.getalldir()
        else:
            self.filelistX = self.getXfiles()
            self.filelistM = self.getMfiles()
    ...
    def getallfiles(self):
        ...
        fM = []
        d = self.basepath + '/' + self.part + '/mask/'
        for root, _, fnames in sorted(os.walk(d)):
            fM.extend(fnames)
    ...
    def getTestbatch(self,i):
        ...
        idfs = list(range(25*5))
        ok = True
        #print(self.basepath + '/' + self.part + '/Y/'+self.filelistX[i])
        #if 1:
        try:
        	
            Xj = pims.Video(self.basepath + '/' + self.part + '/Y/'+self.filelistX[i])[idfs]
            Xj = np.array(Xj, dtype='float32') / 255.
            
            Mj = pims.Video(self.basepath + '/' + self.part + '/mask/'+self.filelistM[i])[idfs]
    ...
    def getTestbatchFrame(self,idx):
        X = []
        M = []
        #Read a batch of clips from files
        for i in idx:
            ...
            try:
                ...
                Mj = []
                for fid in idfs:
                    im = cv2.imread(self.basepath + '/' + self.part + '/mask/'+self.filelistM[i]+'/'+str(fid)+'.jpg')
                    Mj.append(im[...,[2,1,0]])
                    Mj = Mj[...,0]
                    Mj[Mj<=0.6]=0
                    Mj[Mj>0.6]=1
    ...
    def getTrainbatchFrame(self,idx):
        ...
        #print(idx)
        for i in idx:
            #print(i,self.filelistX[i], self.filelistY[i], self.filelistM[i])            
            idfs = list(range(25*5))
            np.random.shuffle(idfs)
            idfs = idfs[:self.no_frames] # keep only 2 random frames per clip on train mode
            ok = True
            ...
            try:
                ...
                for fid in idfs:
                    im = cv2.imread(self.basepath+'/'+self.part+'/Y/'+self.filelistY[i]+'/'+str(fid)+'.jpg')
                    #cv2.imshow("Y",im)
                    #cv2.imwrite( "./gt_orig"+str(i)+".jpg", im)
                    Yj.append(im)
                    #Yj.append(im[...,[2,1,0]])
    ...
    def saveVideo(self, savepath, name, i, clip):
        ...
        '-vframes', '125', # 5*25
        '-s', '128x128', #'256x256', # size of one frame
        savepath+'/'+name+self.filelistX[i][1:]] #savepath+'/Y'+self.filelistX[i][1:]]
